...
In order to use machine learning (ML) in part 4, we need to increase the amount of data, so two functions are defined: one calculates SAVI, and the other retrieves yearly Sentinel-2 images for the analysis over time.
...
We tried various methods to handle the differences between the images, and eventually settled on directly applying subtraction, as it preserves the most changes without being filtered out. Otherwise, we wouldn't be able to detect where new roads have appeared.
...
![part3_2](images/part3_2.jpg)

We can use statistical analysis to observe the distribution of suspected new Israeli settlement developments each year.
